...
declare_id!("4WJpEQhbaxqieuHioFCnupC5W5QebwCd67P4EdTsNuC5");
...
pub mod blog_sol {
    ...
    pub fn create_post(ctx: Context<CreatePost>, title: String, content: String) -> Result<()> {
        ...
        post_account.user = user_account.key(); // user account public key
        post_account.authority = authority.key(); // user wallet public key
        
        // increase post id by 1
        user_account.last_post_id = user_account.last_post_id.checked_add(1).unwrap();
        // increase post totals
...
pub struct CreatePost<'info> {
    #[account(
        init,
        seeds = [POST_SEED, authority.key().as_ref(), &[user_account.last_post_id as u8].as_ref()], // to make post account publik key unique
        bump,
        payer = authority,
        space = 2376 + 8
    )]
